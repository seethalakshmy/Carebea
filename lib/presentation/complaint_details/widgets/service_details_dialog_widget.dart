...
class ServiceDetailsDialogWidget extends StatefulWidget {
  const ServiceDetailsDialogWidget(
      {Key? key, required this.service, required this.title})
      : super(key: key);

  final GetServiceData service;
  final String title;

  @override
  State<ServiceDetailsDialogWidget> createState() =>
      _ServiceDetailsDialogState();
}

class _ServiceDetailsDialogState extends State<ServiceDetailsDialogWidget> {
  ...
  final List<String> servicesNeededList = [];
  final List<String> servicesCompletedList = [];
  final List<String> servicesIncompleteList = [];

  @override
  Widget build(BuildContext context) {
    var size = MediaQuery.of(context).size;
    if (widget.service.serviceNeeded!.tier1 != null) {
      for (int i = 0; i < widget.service.serviceNeeded!.tier1!.length; i++) {
        servicesNeededList
            .add(widget.service.serviceNeeded!.tier1![i].name ?? "");
      }
      for (int i = 0; i < widget.service.serviceNeeded!.tier2!.length; i++) {
        servicesNeededList
            .add(widget.service.serviceNeeded!.tier2![i].name ?? "");
      }
      print("needed services list: $servicesNeededList");
    }

    if (widget.service.servicesCompleted!.tier1 != null) {
      for (int i = 0;
          i < widget.service.servicesCompleted!.tier1!.length;
          i++) {
        servicesCompletedList
            .add(widget.service.servicesCompleted!.tier1![i].name ?? "");
      }
      for (int i = 0;
          i < widget.service.servicesCompleted!.tier2!.length;
          i++) {
        servicesCompletedList
            .add(widget.service.servicesCompleted!.tier2![i].name ?? "");
      }
      print("completed services list: $servicesCompletedList");
    }

    if (widget.service.servicesIncompleted!.tier1 != null) {
      for (int i = 0;
          i < widget.service.servicesIncompleted!.tier1!.length;
          i++) {
        servicesIncompleteList
            .add(widget.service.servicesIncompleted!.tier1![i].name ?? "");
      }
      for (int i = 0;
          i < widget.service.servicesIncompleted!.tier2!.length;
          i++) {
        servicesIncompleteList
            .add(widget.service.servicesIncompleted!.tier2![i].name ?? "");
      }
      print("incomplete services list: $servicesIncompleteList");
    }
                                ...
                                children: [
                                  ProfileWidget(
                                      width: !Responsive.isWeb(context)
                                          ...
                                          : null,
                                      imageUrl:
                                          widget.service.user?.profile ?? "",
                                      name:
                                          "${widget.service.user?.firstName} ${widget.service.user?.lasttName}",
                                      subText: 'Client'),
                                  Row(
                                    crossAxisAlignment:
                                  ...
                                  if (widget.title == AppString.canceled.val)
                                    const CancellationWidget(reason: ""),
                                  _textAndSubText(
                                  ...
                                  /*if (widget.title == AppString.completed.val)
                                    RatingWidget(
                                        service: ServiceList.fromJson(
                                            widget.service.toJson())),*/
                              ...
                              child: Column(
                                children: [
                                  ...
                                  _serviceListWidget(
                                      ...
                                      servicesList: servicesNeededList),
                                  if (widget.title == AppString.completed.val)
                                    _textAndSubText(
                                        text:
                                  ...
                                  if (widget.title == AppString.completed.val)
                                    _textAndSubText(
                                        ...
                                        subText: widget
                                            ...
                                            ''),
                                  if (widget.title == AppString.completed.val &&
                                      (servicesCompletedList).isNotEmpty)
                                    _serviceListWidget(
                                        title: 'Service Completed',
                                        servicesList: servicesCompletedList),
                                  if (widget.title == AppString.completed.val &&
                                      (servicesIncompleteList).isNotEmpty)
                                    _serviceListWidget(
                                        title: 'Service Incomplete',
                                        servicesList: servicesIncompleteList)
                                ],
                              ),
                            )
                          ],
      ...
      onPressed: () {
        ...
        if (_formKey.currentState!.validate()) {
          RescheduleParams rescheduleParams = RescheduleParams(
            ...
            startTime: startTime,
            endTime: endTime,
            addressId: "widget.service.id",
            //genderPreference: "widget.service.genderPreferences",
            profileId: "widget.service.profileID",
            serviceId:
                widget.service.serviceId, /*selectedServices: widget.service.*/
          );
          context.read<ServiceRequestManagementBloc>().add(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            CustomText(
              subText,
              width: Responsive.isWeb(context) ? null : 150,
            )
          ],
        ),
      ],
